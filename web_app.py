...
# Helper to get client from cache or session
def _get_project_client(user_id):
    ctx = get_analysis_context(user_id)
    if ctx and ctx.get('client'):
        return ctx['client']
    
    # Fallback: Try to reconstruct from session
...
def get_project_fields():
    """Retorna lista de campos do projeto (Metadata)."""
    try:
        user_id = session.get('user_id')
        client = _get_project_client(user_id)
        
        if not client:
             return jsonify({'success': False, 'error': 'Sessão expirada. Conecte novamente.'})
        ...
        print(f"Error in /api/fields: {e}")
        return jsonify({'success': False, 'error': str(e)})


@app.route('/api/events', methods=['GET'])
def get_project_events():
    """Retorna lista de eventos do projeto (se longitudinal)."""
    try:
        user_id = session.get('user_id')
        client = _get_project_client(user_id)
        ...
        # Check longitudinal
        info = client.export_project_info()
        is_longitudinal = info.get('is_longitudinal', 0) == 1
        ...
        if is_longitudinal:
            ...
            # If classical, maybe return a single "default" event or empty
            # For Cross-Event logic, having no events means it's disabled effectively
            pass
